...
const BestSellers = () => {
  return (
    <section className="w-[90%] max-w-7xl mx-auto py-12">
      ...
      <div className="grid grid-cols-4 sm:grid-cols-3 lg:grid-cols-4 gap-6 perspective-[1000px]">
        {bestSellers.map((item, index) => (
          ...
          >
            <img
              src={item.img}
              alt={item.title}
              className="w-full h-[250px] object-cover group-hover:brightness-110 transition-all duration-500"
            />
            <div className="absolute bottom-0 w-full bg-black bg-opacity-50 text-white py-2 text-center text-sm font-medium">
              {item.title}
            </div>
          </div>
        ))}
